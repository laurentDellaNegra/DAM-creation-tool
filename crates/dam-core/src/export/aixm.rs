use crate::{
    ...
    ValidationIssue, expand_polygon_nodes, validate_creation,
};
...
mod tests {
    ...
    #[test]
    fn aixm_export_uses_predefined_catalog_fallback_geometry_and_label() {
        let mut creation = valid_creation();
        creation.map = DamMap::Predefined(SelectedStaticMap {
            id: "50714".to_owned(),
            name: "HAUT VALAIS".to_owned(),
            ...
            fallback_label_position: Some(Coordinate {
                lon: 7.25,
                lat: 46.15,
            }),
        });
        ...
        assert!(xml.contains("<gml:posList>7.1 46.1 7.2 46.2 7.3 46.1 7.1 46.1</gml:posList>"));
        assert!(xml.contains("<gml:pos>7.25 46.15</gml:pos>"));
    }

    #[test]
    fn aixm_export_uses_static_fallback_when_predefined_geometry_is_missing() {
        let xml = to_aixm_xml(&valid_creation()).unwrap();
